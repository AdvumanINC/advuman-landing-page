...
function Analytics() {
  ...
  const fetchData = useCallback(async () => {
    const [{ data: disruptions }, { data: risks }, { data: sources }] = await Promise.all([
      supabase.from('disruptions').select('*'),
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'disruptions' }, fetchData)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'risks' }, fetchData)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'sources' }, fetchData)
      .subscribe();
    return () => supabase.removeChannel(channel);
  }, [fetchData]);

  const maxDisruption = Math.max(...disruptionData.map(d => d.count || 0), 1);
  const totalRisk = riskDistribution.reduce((sum, r) => sum + (r.count || 0), 0);
  ...
  return (
    <div style={{ padding: '32px 40px' }}>
            ...
            {disruptionData.map((d, i) => (
              <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
                <div style={{ width: '100%', height: 200, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                  <div style={{ width: '100%', background: 'linear-gradient(180deg, #c8a932 0%, #d4b744 100%)', borderRadius: '4px 4px 0 0', height: `${(d.count / maxDisruption) * 100}%`, position: 'relative' }}>
                ...
                <span style={{ fontSize: 12, color: '#666', fontFamily: FONTS.mono }}>{d.month}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          {riskDistribution.map((r, i) => (
            <div key={i} style={{ marginBottom: 16 }}>
              ...
              <div style={{ width: '100%', height: 8, background: '#07080a' }}>
                <div style={{ width: `${(r.count / totalRisk) * 100}%`, height: '100%', background: r.color }} />
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ background: '#0a0b0e', border: '1px solid #1a1c20', borderRadius: 8, padding: 32 }}>
        <h3 style={{ color: '#f0ede6', marginBottom: 24 }}>Source Monitoring Coverage</h3>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
          {sourceCoverage.map((s, i) => (
            <div key={i} style={{ padding: 20, background: '#07080a' }}>
              <div style={{ color: '#b0ae9f' }}>{s.source}</div>
              <div style={{ fontSize: 28, color: '#f0ede6' }}>{s.alerts}</div>
            </div>
          ))}
        </div>
      </div>
